...
class UserSalaryAssignmentModelTests(TestCase):
    ...
    def test_one_to_one_relationship(self):
        ...
        # При создании нового назначения старое должно удалиться или обновиться
        assignment2 = UserSalaryAssignment.objects.create(
            user=self.user,
            config=config2
        )
        
        # Должно быть только одно назначение для пользователя
        assignments = UserSalaryAssignment.objects.filter(user=self.user)
        self.assertEqual(assignments.count(), 1)
        self.assertEqual(assignments.first().config, config2)
...
class SalaryAdjustmentModelTests(TestCase):
    ...
    def test_adjustment_ordering(self):
        """Тест сортировки корректировок"""
        adjustment1 = SalaryAdjustment.objects.create(
            user=self.user,
            adjustment_type='bonus',
            amount=Decimal('1000.00'),
            reason='Первая премия',
            period_start=datetime(2025, 6, 1).date(),
            period_end=datetime(2025, 6, 30).date(),
            created_by=self.creator
        )
        
        adjustment2 = SalaryAdjustment.objects.create(
            user=self.user,
            adjustment_type='penalty',
        ...
        # По умолчанию сортировка по -created_at
        adjustments = SalaryAdjustment.objects.all()
        self.assertEqual(adjustments[0], adjustment2)  # Последняя созданная должна быть первой
        self.assertEqual(adjustments[1], adjustment1)
